    ...
    namespace conexion
    {
        public class UsuarioDAL
        {
            public Usuario Login(string email, string passHash)
            {
                try
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(
                        "SELECT Id, email, nombre, apellido, urlImagenPerfil, admin " +
                        "FROM dbo.USERS WHERE email=@Email AND pass=@Pass", cn))
                    {
                        cmd.Parameters.Add("@Email", SqlDbType.VarChar, 100).Value = (email ?? "").Trim();
                        cmd.Parameters.Add("@Pass", SqlDbType.VarChar, 200).Value = (passHash ?? "").Trim();
            ...
            public void Registrar(Usuario u)
            {
                try
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("dbo.SP_RegistrarUsuario", con))
                    {
                        ...
                        cmd.Parameters.Add("@Pass", SqlDbType.VarChar, 200).Value = (u.Pass ?? "").Trim();
            ...
            public void ActualizarPerfil(Usuario user)
            {
                ...
                {
                    throw new Exception("Error en UsuarioDAL.ActualizarPerfil", ex);
                }
            }
            ...
            public void ConfirmarReset(string token, string nuevaPass)
            {
                try
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("dbo.SP_Reset_Confirmar", con))
                    {
                        ...
                        cmd.Parameters.Add("@TokenHash", SqlDbType.VarBinary, 32).Value = tokenHash;
                        cmd.Parameters.Add("@NuevaPass", SqlDbType.VarChar, 200).Value = (nuevaPass ?? "").Trim();
            ...
            public bool CambiarPasswordPorEmail(string email, string passHash)
            {
                ...
                {
                    throw new Exception("Error en UsuarioDAL.CambiarPasswordPorEmail", ex);
                }
            }
